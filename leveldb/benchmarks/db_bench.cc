...
// Size of each value
static int FLAGS_value_size = 32;
 ...
 public:
  ...
  void Set(int k) {
    std::snprintf(buffer_ + FLAGS_key_prefix,
                  sizeof(buffer_) - FLAGS_key_prefix, "%032d", k);
  }

  Slice slice() const { return Slice(buffer_, FLAGS_key_prefix + 32); }
...
class Benchmark {
 private:
  ...
  void PrintHeader() {
    const int kKeySize = 32 + FLAGS_key_prefix;
    PrintEnvironment();
  ...
  Benchmark()
      : cache_(NewLRUCache(0)),
        filter_policy_(FLAGS_bloom_bits > 0
                           ...
                           : (FLAGS_bloom_bits == 0 ? nullptr:NewWormholeFilterPolicy())),
        db_(nullptr),
        num_(FLAGS_num),
