...
static std::string Key(int i) {
  ...
  std::string padding=std::string(32-data.size(),'a');
  std::string key = padding+data;
  assert(key.size()==32);
  return key;
}

static std::string Keymissing(int i) {
  char buf[100];
  std::snprintf(buf, sizeof(buf), "key%06d", i);
  std::string data=std::string(buf);
  std::string padding=std::string(32-data.size(),'a');
...
TEST_F(DBTest, BloomFilter) {
  ...
  options.filter_policy = NewWormholeFilterPolicy();
  Reopen(&options);
  ...
  for (int i = 0; i < N; i++) {
    ASSERT_EQ("NOT_FOUND", Get(Keymissing(i)));
  }
